...
void update_run_time(thread_info_t *info) {
	...
	long difference = time_difference(&info->suspend_time, 
						&info->resume_time);
	info->run_time = info->run_time + difference;
}
...
void update_wait_time(thread_info_t *info) {
	...
	long difference = time_difference(&info->resume_time,
						&info->suspend_time);
	info->wait_time = info->wait_time + difference;
}
...
void setup_sig_handlers() {
	...
	sa_alrm.sa_flags = 0;
	sigaction(SIGALRM, &sa_alrm, NULL); //trigger timer_handler()
	
	/* Setup cancel handler for SIGTERM signal in workers */
	struct sigaction st_alrm;
	st_alrm.sa_handler = cancel_thread;
	sigemptyset(&st_alrm.sa_mask);
	st_alrm.sa_flags = 0;
	sigaction(SIGTERM, &st_alrm, NULL); //trigger cancel_thread()
	
	/* Setup suspend handler for SIGUSR1 signal in workers */
	struct sigaction su_alrm;
	su_alrm.sa_handler = suspend_thread;
	sigemptyset(&su_alrm.sa_mask);
	su_alrm.sa_flags = 0;
	sigaction(SIGUSR1, &su_alrm, NULL); //trigger suspend_thread()
}
